...
//Default ahh helloHono print pannum
app.get('/', (c) => {
...
//Ethu users ahh post panna ennum Ui panneyachu 
//with existing user check
app.post('/users', async (c) => {
  try {
    ...
    ).bind(random_id, username, email).all() as unknown as Array<any>;

    if (existingUser.length > 0) {
      return c.json({ error: "User already exists" });
    }
    // Add the user to the database using parameterized query
    const result = await c.env.DB.prepare(
      `INSERT INTO users (user_id, username, email, password) VALUES (?,?,?,?)`
    ).bind(random_id, username, email, password).run();

    // Check if the insertion was successful
    if (result) {
      return c.json({ message: "User created successfully",random_id });
    } else {
      return c.json({ error: "Error adding user" });
    }
  } catch (error) {
    
    if ((error as Error).message.includes("UNIQUE constraint failed: users.user_id")) {
      return c.json({ error: "NUID Conflict kuda namba project ladha agum" })}


    if((error as Error).message.includes("UNIQUE constraint failed: users.username")){
      return c.json({ error: "User Name already exists"})
    }
...
/*app.post('/users', async (c) => {
  try {
    const { user_id, username, email, password }: User = await c.req.json();

    // Check if the user already exists in the database
    const existingUser = await c.env.DB.prepare(
      `SELECT * FROM users WHERE user_id = ? OR username = ? OR email = ?`
    ).bind(user_id, username, email).all() as unknown as Array<any>;
...
//Login ahh checkpannum like instagram email or userName and password
...
//Ethu skills ahh post panna ennum UI pannala POstman dhan
...
//ethu reqeuest ahh post panna ennum UI pannala POstman dhan
...
//ethu user_skills ahh post panna ennum UI pannala POstman dhan
...
//ethu user_skills ahh get panna ui panneyachu
// ennum home page
app.get('/user_skills/:id', async (c) => {
  try {
    ...
    const userSkills = await c.env.DB.prepare(`
      ...
      WHERE us.user_id != ? AND NOT EXISTS (
        SELECT 1 FROM requests r
        WHERE r.receiver_id = us.user_id AND r.status = 'approved'
      )
    `).bind(id).all();
...
//Pending status ahh reject panna delete pannavum approve pannavum
...
// request ahh get panna receiver details and skill names UI panneyachu by prithiviraj
//correction mady by Rishi(actually the checking condition for  WHERE r.receiver_id = ? AND r.status!="approved)
...
/*
app.get('/todos/sql/allsearch'),async(c)=>{
  const query = c.req.query('title')


  const tasks = await c.env.DB.prepare(`
  SELECT * FROM tasks WHERE title LIKE ?;`).bind(`%${query}%`).all()
  ...
  app.post('/user_skillsUI', async (c) => {
    try {
      ...
      if (existingSkill.results.length>0) {
        // If the skill already exists, use its skill_id
        skill_id = existingSkill.results[0].skill_id;
        //skill_id = existingSkill.results;
        ...
        // If the skill does not exist, insert it into the skills table and get the new skill_id
        const randomid = nanoid(5)
        const randomdescription = nanoid(3)
        const result = await c.env.DB.prepare(
          `INSERT INTO skills (skill_id,name,description) VALUES (?,?,?)`
        ).bind(randomid,skill,randomdescription).run();

        const lastInsertedSkillId = await c.env.DB.prepare(
          `SELECT skill_id FROM skills WHERE name = ?;`
        ).bind(skill).all();
        
        // Retrieve the last inserted row ID using lastInsertRowid()
        skill_id = lastInsertedSkillId.results[0].skill_id;
        await c.env.DB.prepare(
          `INSERT INTO user_skills (user_id, skill_id) VALUES (?,?)`
        ).bind(user_id, skill_id).run();
      }
      // Add the user skill to the user_skills table using parameterized query
